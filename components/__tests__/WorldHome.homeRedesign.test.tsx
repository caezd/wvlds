...
describe("WorldHome — titre/description hors bannière + panel de contenu", () => {
  ...
  it("colle la barre de boutons en haut du conteneur qui défile, sans lui prendre de place", () => {
    ...
    // Le repère qui déclenche le header couvre tout le bloc hero (bannière,
    // titre, description) : le header n'apparaît qu'une fois celui-ci passé.
    const titleBlock = screen.getByRole("heading", { name: "Avalonia" }).closest("div.relative")!;
    const sentinel = titleBlock.parentElement!.querySelector("div.absolute.inset-0[aria-hidden]");
