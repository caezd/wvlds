-- Fix : le tri de get_dm_conversations() utilisait c.last_message_at qui peut
-- être NULL si le client émetteur n'a pas exécuté le UPDATE (race condition).
-- On trie désormais par le MAX réel des timestamps dans dm_messages, ce qui
-- reste correct même si last_message_at n'a jamais été mis à jour.
