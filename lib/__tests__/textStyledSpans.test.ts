...
describe("transformStyledSpans", () => {
  ...
  it("transforme un span souligné en lien underline:", () => {
    expect(transformStyledSpans("{u}texte souligné{/}")).toBe("[texte souligné](underline:)");
  });
  ...
  it("gère plusieurs spans sur la même ligne", () => {
    expect(transformStyledSpans("{#ff0000}a{/} et {u}b{/}")).toBe("[a](color:ff0000) et [b](underline:)");
  });
